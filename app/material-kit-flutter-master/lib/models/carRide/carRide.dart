...
@JsonSerializable(disallowUnrecognizedKeys: false, checked: false, includeIfNull: false)
class CarRide {
  ...
  CarRide(
      ...
      required this.tip,
      this.driver, required this.finalComments, required this.organization, this.user});
  ...
  String toString() {
    return 'CarRide{id: $id, passenger: $passenger, driver: $driver, '
        ...
        ' observationsDriver: $observationsDriver, tip: $tip, '
        'finalComments: $finalComments, organization: $organization, user: $user}';
  }

  static Future<CarRide> registerCarRide(Map<String, dynamic> mapData) async {
    Map<String, dynamic> response =
        await DioClient().postJsonRequest('/carRides/create', mapData,
  ...
  static Future<CarRide> getCarRide(String id) async {
    Map<String, dynamic> response =
        await DioClient().getJsonRequest('/carRides/$id', tokenValue: await getSecureStorage('token'));
  ...
  static CarRide defaultCarRide() {
    return CarRide(
      ...
      observationsDriver: {},
        tip: "0",
        driver: "", finalComments: {}, organization: ''
        );
  }

  static Future<bool> deleteRequest(String? id) async {
    Map<String, dynamic> response =
        await DioClient().deleteJsonRequest('/carRides/$id', {}, tokenValue: await getSecureStorage('token'));
    if (!response['error']) return true;
    return false;
  }

  static Future<bool> configDriver(String? id, String driver,
      Map<String, dynamic> observationsDriver, String status, {bool asDriver = false}) async {
  ...
  static Future<List<CarRide>> getCarRideAll(String username, String organization) async {
    List<dynamic> response = await DioClient().getJsonListRequest(
        '/carRides/all/$username/$organization',
        tokenValue: await getSecureStorage('token'));
    return response.map((entry) => CarRide.fromJson(entry)).toList();
  }

  static Future<List<CarRide>> getCarRideAllDriver(String organization, String status)
    async {
    List<dynamic> response = await DioClient().getJsonListRequest(
        '/carRides/availiable/$organization/$status',
        tokenValue: await getSecureStorage('token'));
    return response.map((entry) => CarRide.fromJson(entry)).toList();
  }
}
